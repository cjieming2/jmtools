...
sub motif_unfixed{
	...
	if (scalar @des >0){
		...
		#positive strand
			&seq_scan(\@extract_seq,"+",$start,$alt,$ref,\%motif,\%score,$id,\%score_upper,\%score_lower,$p_cut);
...
# sub-routine for the sequence scanning ... 
	...
	sub seq_scan{
		...
		my ($seq, $strand,$start,$alt,$ref,$motif,$score,$id,$score_upper,$score_lower,$p_cut) = @_;
		my @seq;
		my $alt_pwm_score;
		my $ref_pwm_score;
		my $motif_length;
		...
		foreach my $prev_name(sort keys %motif){
			...
			for ($i=30 - $motif_length; $i < 30; $i ++){
				...
				if (defined $score{$prev_name}){
					...
					my $max_score = max($alt_pwm_score,$ref_pwm_score);
					if (defined $score_upper{$prev_name} && $max_score >= $score_upper{$prev_name}){
						&out_print($prev_name,$i,$motif_length,$alt_pwm_score,$ref_pwm_score,$strand,$id,$start);  
					}elsif(defined $score_lower{$prev_name} && $max_score<=$score_lower{$prev_name}){
						#next;
					}elsif($max_score >$score_lower{$prev_name}||'0' && $max_score < $score_upper{$prev_name}||'40'){	
						$p_value = (split /\s+/,`TFMpvalue-sc2pv -a 0.25 -c 0.25 -g 0.25 -t 0.25 -s $max_score -m pwm/$prev_name -w`)[2];
						if ($p_value < $p_cut){
							&out_print($prev_name,$i,$motif_length,$alt_pwm_score,$ref_pwm_score,$strand,$id,$start);	
						}
					}	
...
sub out_print{
	...
	if ($strand eq "+"){
		...
		print $id,"\t",sprintf('%.3f', $ref_pwm_score),"\t",sprintf('%.3f', $alt_pwm_score),"\t",$tmp,"\n";
	}else{
