...
echo -e '           '\<\(\$\(PL\)\/filter_input.sh \$\(PL\) \$\< \| bowtie --best --strata -v 2 -m 1 -f \$\(BASE\)\/AltRefFather\/AltRefFather - \) \\ >> PIPELINE.mk
echo -e '           '\<\(\$\(PL\)\/filter_input.sh \$\(PL\) \$\< \| bowtie --best --strata -v 2 -m 1 -f \$\(BASE\)\/AltRefMother\/AltRefMother - \) \\ >> PIPELINE.mk
